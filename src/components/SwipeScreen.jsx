...
import { fetchFoodDeck } from "../data/foods";
...
export default function SwipeScreen({
  ...
  useEffect(() => {
    fetchFoodDeck(12, categories)
      .then((deck) => setFoods(deck))
      .catch((err) => {
        console.error("Failed to fetch food deck:", err);
        setFoods([]);
      })
  ...
  const vote = useCallback(
    async (likedVote) => {
      if (isExiting || !currentFood) return;
      ...
      setTimeout(async () => {
        if (currentIndex < foods.length - 1) {
          ...
          const finalLiked =
            likedVote === true ? [...liked, currentFood] : liked;
  ...
  }, [isDragging, isExiting, dragX, vote]);
  ...
  if (loading) {
    return (
      <div className="bg-[#4A4A4A] pixel-grid h-screen overflow-hidden text-on-surface font-body flex flex-col items-center justify-center gap-8">
        <div
          style={{ animation: "spin 1s steps(4) infinite", fontSize: "3rem" }}
        >
          ??
        </div>
          ...
          <div className="flex flex-col gap-4">
            {Object.entries(roomPlayers).map(([id, player], i) => (
              <div key={id} className="flex items-center gap-4">
                ...
                <span
                  className={`font-headline text-[10px] ${player.finished ? "text-[#5D8A32]" : "text-[#7F7F7F]"}`}
                >
        ...
        <>
          ...
          <header className="fixed top-0 left-0 right-0 h-20 bg-[#efeded] border-b-4 border-[#1a1a1a] shadow-[4px_4px_0px_#1a1a1a] px-6 flex justify-between items-center z-50">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="material-symbols-outlined text-[#3c6711] text-[2rem]">
                  arrow_back
                </span>
              </button>
              <span className="text-[0.875rem] text-[#3c6711] uppercase font-headline">
                {getModeLabel()}
              </span>
            </div>
            ...
            <div className="max-w-[200px] w-full mx-4 flex-1">
              <div className="xp-bar-container bg-[#313131] h-6 w-full relative">
                <div
                  ...
                  style={{
                    width: `${foods.length ? (currentIndex / foods.length) * 100 : 0}%`,
                  }}
                />
                <span className="absolute inset-0 flex items-center justify-center text-[10px] text-white drop-shadow-[2px_2px_0px_#000] font-headline">
                  {foods.length ? currentIndex + 1 : 0}/{foods.length}
                </span>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              >
                <div className="rotate-[-12deg] bg-primary border-8 border-on-primary-container p-4 pixel-shadow-box">
                  <span className="text-white text-2xl font-headline">
                    ? YUM!
                  </span>
                </div>
              </div>
              ...
              {currentFood && (
                ...
                >
                  <div className="flex-1 border-4 border-[#7b563a] bg-surface-container-lowest flex flex-col">
                    <div
                      style={{ height: "300px" }}
                      className="dithered-bg relative overflow-hidden"
                    >
                      <img
                      ...
                      <div className="absolute top-4 right-4 bg-tertiary px-3 py-1 border-2 border-black">
                        <span className="text-[10px] text-white uppercase font-headline">
                          {currentFood.tags[0]}
                        </span>
                      </div>
                    </div>
                    ...
                    <div
                      className="bg-[#efeded] p-3 flex flex-col justify-start border-t-4 border-[#1a1a1a]"
                      style={{ flexShrink: 0 }}
                    >
                      <h2 className="text-on-surface text-sm font-headline mb-2 leading-tight uppercase">
                        {currentFood.name}
                      </h2>
                      <div className="flex flex-wrap gap-2">
                        {currentFood.tags.map((tag, i) => (
                          <span
                            key={i}
                            className="bg-[#1a1a1a] text-white text-[8px] px-2 py-1 uppercase font-headline"
                          >
                            {tag}
                          </span>
                        ))}
            ...
            <div
              ...
              style={{ flexShrink: 0, paddingTop: "12px" }}
            >
              <button
                onClick={() => vote(false)}
              ...
              >
                <span className="material-symbols-outlined bold text-white text-2xl">
                  close
                </span>
              </button>
              <button
